...
module Validator
  ...
  module ClassMethods
    def validate (attribute, rule_name, details = nil)
      class_variable_set(:@@rules, []) unless class_variable_defined?(:@@rules)
      ...
      class_variable_get(:@@rules) << validation
    end
  ...
  module InstanceMethods
    def validate!
      validations = self.class.class_variable_get(:@@rules)

      validations.each do |validation|
        rule = validation[:rule_name]
        attribute_value = instance_variable_get("@#{validation[:attribute]}")

        self.class.send(rule, validation, attribute_value)
      end
    end
